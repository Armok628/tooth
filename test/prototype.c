...
CWORD(&f_divmod.link,"\0021+",f_incr) { sp[0]++; next(); }
CWORD(&f_incr.link,"\0021-",f_decr) { sp[0]--; next(); }
CWORD(&f_decr.link,"\006NEGATE",f_negate) { sp[0]=-sp[0]; next(); }
CWORD(&f_negate.link,"\006INVERT",f_invert) { sp[0]=~sp[0]; next(); }
/********************************/
#define CMPOP(op) { \
	register cell b=pop(); \
	sp[0]=sp[0] op b?~0:0; \
